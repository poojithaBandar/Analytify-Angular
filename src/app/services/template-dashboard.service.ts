...
export class TemplateDashboardService {
  ...
  buildSampleGeminiDashboard(container: ViewContainerRef , databaseId: any, responceData?: any){
    const componentRef =container.createComponent(InsightEchartComponent);
    this.echartInstance = componentRef.instance;
    ...
    if(responceData){
      ...
      this.workbechService.buildSampleGeminiDashboard(databaseId).subscribe({
        next: handleResponse,
        error: (error) => {
          this.toasterservice.error(error.error.message,'error',{ positionClass: 'toast-center-center'})
          console.log(error);
