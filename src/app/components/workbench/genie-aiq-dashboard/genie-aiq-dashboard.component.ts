...
export class GenieAiqDashboardComponent {
  ...
  @ViewChild('sheetcontainer', { read: ViewContainerRef })
  ...
  constructor(
    ...
    private dashbaordTransferService: DashboardTransferService
  ) {}
  ngOnInit() {
    this.step = 1;
  ...
  removeSelectedInsight(insight: any) {
    this.selectedInsights = this.selectedInsights.filter(
      (i) => i.sample_id !== insight.sample_id
    );
  }
  ...
  generateDashboard() {
    if (!this.selectedInsights.length) {
      this.toasterService.error(
        'Please select at least one chart to generate dashboard',
        'Error'
      );
      return;
    }
    ...
    const hierarchy_id =
      this.suggestedInsights[0]?.hierarchy_id || this.hierarchyId || null;
    ...
    this.workbechService.getDashbaordSuggestions(payload).subscribe({
      next: (data) => {
        console.log(data);
        this.templateDashboardService.buildSampleGieneAiqDashbaord(
          this.container,
  ...
  isCreateDisabled(): boolean {
    ...
    if (this.selectedCard === 'insight' && this.selectedTables.length === 0) {
      return true;
    }
    return false;
  }
    ...
    if (this.selectedCard === 'prompt') {
      ...
      this.workbechService.promptDashboard(payload).subscribe({
        next: (data) => {
          ...
          this.dashboardName =
            this.datafromApi.dashboard.dashboard.dashboard_title;
  ...
  buildGenieDashbaordJson() {
    this.datafromApi.dashboard_json = this.datafromApi.dashboard_json.map(
      (chartItem: any) => {
        const title = chartItem?.data?.title?.trim();
        const sheet = this.datafromApi.dashboard_data.sheets.find(
          (s: any) => s.sheet_name === title
        );
        ...
        if (chartItem.chartId === 1) {
          ...
          const rows =
            combined[0]?.result?.map((_: any, index: number) => {
              const obj: any = {};
  ...
  jsonTosendBackToAPI() {
    ...
    console.log(
      'Cleaned response to send back to API:',
      this.responceTosendBackToApi
    );
    return cleaned;
  ...
  builSheets(data: any, dashboard: any) {
    ...
    from(dashboard)
      .pipe(
        concatMap(
          (sheetData: any, index: number) => {
            if (sheetData.chartType === 'text' || sheetData.type === 'text') {
              return of({ res: null, index });
            }
            const currenrSheet = data.find(
              (f: any) => f.sheet_name === sheetData.data.title
            );
  ...
  sheetSave(sheetData: any, data: any, index: any) {
    ...
    if (sheetData.chart_id == 8) {
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.multiLineChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      } else {
        chartData = this.apexChartInstance.multiLineChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.pieChart(
          chartsColumnData,
          chartsRowData
        );
      } else {
        chartData = this.apexChartInstance.pieChart(
          chartsColumnData,
          chartsRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.barChart(
          chartsColumnData,
          chartsRowData
        );
      } else {
        chartData = this.apexChartInstance.barChart(
          chartsColumnData,
          chartsRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.hgroupedChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      } else {
        chartData = this.apexChartInstance.hGroupedChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.donutChart(
          chartsColumnData,
          chartsRowData
        );
      } else {
        chartData = this.apexChartInstance.donutChart(
          chartsColumnData,
          chartsRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.sidebySide(
          dualAxisColumnData,
          dualAxisRowData
        );
      } else {
        chartData = this.apexChartInstance.sideBySide(
          dualAxisColumnData,
          dualAxisRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.funnelchart(
          dualAxisColumnData,
          dualAxisRowData
        );
      } else {
        chartData = this.apexChartInstance.funnelChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.mapChart(
          dualAxisColumnData,
          dualAxisRowData,
          chartsRowData
        );
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.hstackedChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      } else {
        chartData = this.apexChartInstance.hStockedChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.linechartFromGenieDashboard(
          chartsColumnData,
          chartsRowData
        );
      } else {
        chartData = this.apexChartInstance.lineChart(
          chartsColumnData,
          chartsRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.areachartFromGenieDashboard(
          chartsColumnData,
          chartsRowData
        );
      } else {
        chartData = this.apexChartInstance.areaChart(
          chartsColumnData,
          chartsRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.barLinechartFromGenieDashboard(
          dualAxisColumnData,
          dualAxisRowData
        );
      } else {
        chartData = this.apexChartInstance.barLineChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.heatmapFromGenieDashboard(
          dualAxisColumnData,
          dualAxisRowData
        );
      } else {
        chartData = this.apexChartInstance.heatMapChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.treemapFromGenieDashboard(
          chartsColumnData,
          chartsRowData
        );
      } else {
        chartData = this.apexChartInstance.treeMapChart(
          chartsColumnData,
          chartsRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.stackedchartFromGenieDashboard(
          dualAxisColumnData,
          dualAxisRowData
        );
      } else {
        chartData = this.apexChartInstance.stockedChart(
          dualAxisColumnData,
          dualAxisRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.calendarchartFromGenieDashboard(
          chartsColumnData,
          chartsRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        ...
        chartData = this.eChartInstance.horizontalBarChart(
          chartsColumnData,
          chartsRowData
        );
      } else {
        this.apexChartInstance.autoAdjustChartHeightForHBar();
        chartData = this.apexChartInstance.horizontalBarChart(
          chartsColumnData,
          chartsRowData
        );
      }
      ...
      if (sheetData?.isEChart) {
        chartData = this.eChartInstance.radarchartFromGenieDashboard(
          dualAxisColumnData,
          dualAxisRowData
        );
      }
    ...
    if (sheetData?.chartId == 25) {
      ...
      sheetData.customizeOptions.kpiShowTrendline =
        sheetData?.kpiData?.kpiShowTrendline;
    }
      ...
      data: {
        ...
        col:
          data?.columns.map((col: any) => {
            return { column: col.column, result_data: col.result };
          }) ?? [],
        row:
        ...
        results: {
          kpiData:
            sheetData?.chartId === 25
              ? data?.rows.map((col: any) => {
                  return { column: col.column, result_data: col.result[0] };
                })[0]
              : [],
          kpiFontSize:
            sheetData?.chartId === 25 ? sheetData?.kpiData?.fontSize : 3,
          kpiNumber:
            sheetData?.chartId === 25 ? sheetData?.kpiData?.kpiNumber : '',
          kpiPrefix:
            sheetData?.chartId === 25 ? sheetData?.kpiData?.kpiPrefix : '',
          kpiSuffix:
            sheetData?.chartId === 25 ? sheetData?.kpiData?.kpiSuffix : '',
          kpiDecimalPlaces:
            sheetData?.chartId === 25
              ? sheetData?.kpiData?.kpiDecimalPlaces
              : 2,
          kpiDecimalUnit: 'none',
          kpicolor:
            sheetData?.chartId === 25 ? sheetData?.kpiData?.color : '#000',
          kpiShowTrendline:
            sheetData?.chartId === 25
              ? sheetData?.kpiData?.kpiShowTrendline
              : false,
          kpiTarget:
            sheetData?.chartId === 25 ? sheetData?.kpiData?.kpiTarget : '',
          trendData:
            sheetData?.chartId === 25 ? sheetData?.kpiData?.trendData : [],
          trendLabels:
            sheetData?.chartId === 25 ? sheetData?.kpiData?.trendLabels : [],
          indicatorIsIncreased:
            sheetData?.chartId === 25
              ? sheetData?.kpiData?.indicatorIsIncreased
              : 'up',
          indicatorValue:
            sheetData?.chartId === 25 ? sheetData?.kpiData?.indicatorValue : '',
          showKpiIndicator:
            sheetData?.chartId === 25
              ? sheetData?.kpiData?.showKpiIndicator
              : false,

          tableData:
            sheetData?.chartId === 1 ? sheetData?.tableData?.headers : [],
          tableColumns:
            sheetData?.chartId === 1 ? sheetData?.tableData?.rows : [],
          banding:
            sheetData?.chartId === 1 ? sheetData?.tableData?.banding : false,
          color1:
            sheetData?.chartId === 1 ? sheetData?.tableData?.color1 : '#f5f5f5',
          color2:
            sheetData?.chartId === 1 ? sheetData?.tableData?.color2 : '#ffffff',
          items_per_page:
            sheetData?.chartId === 1
              ? sheetData?.tableData?.tableItemsPerPage
              : 10,
          total_items:
            sheetData?.chartId === 1
              ? sheetData?.tableData?.tableTotalItems
              : 0,
        },
    ...
    this.workbechService.saveDashboard(object).subscribe({
      next: (res) => {
        ...
        this.router.navigate([
          '/analytify/home/sheetsdashboard/' + encodedDashboardId,
        ]);
    ...
    dashboard: {
      ...
      sheets: [
        ...
        },
        {
          ...
          sql_query:
            'SELECT "Channel", sum("Revenue") FROM (SELECT "Sales Data"."Transaction ID" AS "Transaction ID", "Sales Data"."Date & Time" AS "Date & Time", "Sales Data"."Customer" AS "Customer", "Sales Data"."Product" AS "Product", "Sales Data"."Sales Rep" AS "Sales Rep", "Sales Data"."Region" AS "Region", "Sales Data"."Revenue" AS "Revenue", "Sales Data"."Cost" AS "Cost", "Sales Data"."Profit" AS "Profit", "Sales Data"."Quantity" AS "Quantity", "Sales Data"."Margin %" AS "Margin %", "Sales Data"."Channel" AS "Channel" FROM "Financial_Sales_Data_4KPSSN.xlsx"."Sales Data" AS "Sales Data") temp_table GROUP BY "Channel" ORDER BY "Channel" ASC NULLS FIRST',
          dimensions: ['Channel'],
          metrics: ['sum(Revenue)'],
          chart_type: 'TABLE',
          chart_id: 1,
        ...
        },
        {
          ...
          sql_query:
            'SELECT sum("Profit") FROM (SELECT "Sales Data"."Transaction ID" AS "Transaction ID", "Sales Data"."Date & Time" AS "Date & Time", "Sales Data"."Customer" AS "Customer", "Sales Data"."Product" AS "Product", "Sales Data"."Sales Rep" AS "Sales Rep", "Sales Data"."Region" AS "Region", "Sales Data"."Revenue" AS "Revenue", "Sales Data"."Cost" AS "Cost", "Sales Data"."Profit" AS "Profit", "Sales Data"."Quantity" AS "Quantity", "Sales Data"."Margin %" AS "Margin %", "Sales Data"."Channel" AS "Channel" FROM "Financial_Sales_Data_4KPSSN.xlsx"."Sales Data" AS "Sales Data") temp_table',
          dimensions: [],
          metrics: ['sum(Profit)'],
          chart_type: 'KPI',
          chart_id: 25,
      ...
      overall_insights:
        'The dashboard provides insights into revenue distribution by region and channel, profit trends over time, and key performance indicators for total revenue and profit.',
    },
      ...
      sheets: [
        {
          ...
          columns: [
            {
              column: 'Region',
              result: ['Central', 'East', 'North', 'South', 'West'],
              data_type: 'String',
              type: [],
            },
          ],
          rows: [
            {
              column: 'Revenue',
              result: [
                1007605.4000000001, 558180.26, 839993.2799999999,
                1281203.4100000001, 1159773.1800000002,
              ],
              data_type: 'Float64',
              type: ['sum'],
            },
          ...
          rows: [
            {
              column: 'Profit',
              result: [
                7273.68, 5303.57, 9016.14, 5291.26, 12673.2, 246349.64, 5931.54,
                200390.51, 12098.32, 9795.57, 7486.13, 8549.97, 8234.19,
                10676.15, 10408.77, 13002.37, 11212.4, 10968.4, 4954.23,
                4428.95, 9916.09, 9547.26, 16802.31, 6020.05, 8138.17, 8106.61,
                4025.65, 11533.94, 113922.06, 6561.72, 9046.25, 139594.91,
                5915.85, 10173.62, 10043.43, 142959.24, 7417.74, 10890.23,
                8274.12, 11997.94, 11549.02, 94984.44, 12369.53, 11372.75,
                4207.78, 8225.1, 12194.43, 6691.08, 5584.92, 157445.83,
                10480.35, 10274.78, 6013.44, 8731.11, 10857.55, 3063.66,
                6583.06, 10896.15, 5779.4, 4190.88, 13868.36, 14073.16,
                190737.62, 7524.7, 13817.22, 8065.13, 7512.96, 7228.63,
                13520.83, 4792.67, 2888.65, 7785.82, 224273.25, 9812.93,
                6263.85, 14840.75, 4225.19, 7679.51, 10765.21, 4846.43,
                10015.69, 10160.99, 5561.69, 6566.26, 5865.62, 14338.38,
                7073.82, 4738.56, 10050.21, 4432.27, 10043.54, 11177.2, 5993.19,
                16326.81, 11260.79, 4158.25, 7977.42, 7361.67, 198519.24,
                145464.91,
              ],
              data_type: 'Float64',
              type: ['sum'],
        ...
        },
        {
          ...
          columns: [
            {
              column: 'Channel',
              result: ['Direct Sales', 'Online', 'Partner', 'Retail'],
              data_type: 'String',
              type: [],
            },
          ],
          rows: [
            {
              column: 'Revenue',
              result: [
                1396222.52, 678237.8400000001, 1656015.34, 1116279.8299999996,
              ],
              data_type: 'Float64',
              type: ['sum'],
            },
    ...
    dashboard_json: [
      {
        ...
        column_Data: ['Region', 'sum(Revenue)'],
        row_Data: [
          ['North', 5000],
          ['South', 5000],
          ['East', 4000],
        ...
        chartOptions: {
          ...
          colors: ['#1d2e92', '#088ed2', '#2392c1', '#4CAF50', '#FF9800'],
          dataLabels: {
            enabled: true,
            style: {
              fontSize: '12px',
      ...
      },
      {
        ...
        column_Data: ['Date & Time', 'sum(Profit)'],
        row_Data: [
          ['North', 5000],
          ['South', 5000],
          ['East', 4000],
      ...
      },
      {
        ...
        kpiData: {
          ...
          trendData: [400000, 420000, 430000, 450000],
      ...
      },
      {
        ...
        kpiData: {
          ...
          trendData: [1400000, 1450000, 1500000],
          trendLabels: ['Q1', 'Q2', 'Q3'],
          kpiShowTrendline: true,
          showKpiIndicator: true,
          indicatorIsIncreased: 'up',
          indicatorValue: 5,
